...
class CoquiEngine(BaseEngine):
    ...
    def __init__(self, 
        ...
        self.checkpoint_dir = local_models_path
        if not local_models_path:
            from TTS.utils.generic_utils import get_user_data_dir
            self.checkpoint_dir = os.path.join(get_user_data_dir("tts"), model_name.replace("/", "--")) 
    ...
    def _synthesize_worker(conn, model_name, voice: Union[str, List[str]], language, ready_event, loglevel, speed, thread_count, stream_chunk_size, full_sentences, overlap_wav_len, temperature, length_penalty, repetition_penalty, top_k, top_p, enable_text_splitting, use_mps, local_model_path, use_deepspeed, voices_path, predefined_voices):
        ...
        Args:
            ...
            voice (str): Name to the file containing the voice to clone.
            language (str): Language to use for the coqui model.
        ...
        logging.debug(f"Initializing coqui model {model_name} with cloning reference {voice} and language {language}")
        ...
        try:
            ...
            gpt_cond_latent, speaker_embedding = get_conditioning_latents(voice)
        ...
        try:
            while True:
                ...
                if command == 'update_reference':
                    new_voice = data['cloning_reference_wav']
                    logging.info(f'Updating reference WAV to {new_voice}')                    
                    gpt_cond_latent, speaker_embedding = get_conditioning_latents(new_voice)
                    conn.send(('success', 'Reference updated successfully'))

                # if command == 'update_speaker':
                #     speaker_id = data['speaker_id']
                #     logging.info(f'Updating speaker to {speaker_id}')
                #     try:                    
                #         from TTS.tts.layers.xtts.xtts_manager import SpeakerManager
                #         speaker_file_path = os.path.join(checkpoint_dir, "speakers_xtts.pth")
                #         speaker_manager = SpeakerManager(speaker_file_path)
                #         gpt_cond_latent, speaker_embedding = speaker_manager.speakers[speaker_id].values()
                #         conn.send(('success', 'Speakers updated successfully'))
                #     except Exception:
                #         conn.send(('failed', 'Speakers update failed'))
    ...
    def _set_cloning_reference(self, cloning_reference_wav: str):
        """
    ...
    def set_voice(self, voice: str):
        ...
        """
        self._set_cloning_reference(voice)
